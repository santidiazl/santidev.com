...
A new message would need to include a <code>read</code> boolean column that is false by default but is set to true if the recipient makes the chat from its sender active or if that chat is already active.

This involved writing a new PUT API request that would be made in such cases in order to mark the <code>read</code> property to true in the database.
